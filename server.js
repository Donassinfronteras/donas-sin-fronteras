...
// --- Middleware ---
// The render platform serves static files automatically.
app.use(express.json()); // To parse JSON bodies


// --- API Endpoint ---
// Group routes for the same endpoint for better organization and to prevent routing issues.
app.route('/api/carta-del-dia')
  .get((req, res) => {
    // This GET handler prevents direct browser access confusion.
    res.setHeader('Content-Type', 'application/json');
    res.status(405).json({ // 405 Method Not Allowed
      error: 'Método no permitido.',
      message: 'Este es un punto de conexión de API y no está diseñado para ser accedido directamente desde un navegador.',
      instructions: 'Por favor, visita la aplicación principal y usa la funcionalidad de "Revelar mi Carta" para interactuar con la API correctamente.',
    });
  })
  .post(async (req, res) => {
    // This POST handler generates the card reading.
    try {
